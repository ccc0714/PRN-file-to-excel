...
def compute_summary(x_data):
    ...
    # Total trials = all food + no-food deliveries across L, C, R
    total_food    = summary["L_FOOD"]    + summary["C_FOOD"]    + summary["R_FOOD"]
    total_no_food = summary["L_NO_FOOD"] + summary["C_NO_FOOD"] + summary["R_NO_FOOD"]
    summary["TRIALS"] = total_food + total_no_food
    


    # Proportion correct = correct food deliveries / total trials
    summary["PROP_CORRECT"] = (
        total_food / summary["TRIALS"]
        if summary["TRIALS"] > 0 else 0
    )

    # Per-side proportions (food trials / total trials for each side)
    ...
    summary["TRIAL NUMBER"] = l_total + c_total + r_total

    return summary

# ── Excel writer ─────────────────────────────────────────────────────────────

def write_sheet(ws, data, summary_format="proportion_correct"):
    """
    Write parsed PRN data to an openpyxl worksheet.

    summary_format:
        "proportion_correct"  : L/C/R food counts + single Proportion Correct
        "three_proportions"   : L/C/R trial counts + per-side proportions
    """
    # Fonts / styles
    ...
    if summary_format == "proportion_correct":
        # ── Format A: food delivery counts + single Proportion Correct ──
        # Col H: label   Col I: count
        # L FOOD / L NO FOOD / C FOOD / C NO FOOD / R FOOD / R NO FOOD
        # (blank row)
        # Trials  N     Proportion Correct =  0.9875
        food_rows =  [
        ("L FOOD TRIAL",   summary["L_FOOD_TRIAL"],
         "L FOOD",         summary["L_FOOD"],
         "Proportion Correct", summary["PROP_CORRECT"]),
        ...
        for i, row_data in enumerate(food_rows):
            r = x_data_start + i
        ...
        w(r_trial, 10, summary["TRIAL NUMBER"])
        w(r_trial, 13, "Trials")
        w(r_trial, 14, round(summary["TRIALS"], 4))

    else:
        # ── Format B: per-side trial counts + three proportions ──
        # Col H: trial label   Col I: trial count
        # Col L: food label    Col M: food count
        # Col P: proportion label  Col Q: proportion value
        sum_rows = [
        ("L FOOD TRIAL",   summary["L_FOOD_TRIAL"],
         "L FOOD",         summary["L_FOOD"],
...
def prn_to_excel(prn_path, xlsx_path=None, sheet_name=None, summary_format="proportion_correct"):
    """
    ...
    Args:
        ...
        summary_format: "proportion_correct" or "three_proportions"
    """
    ...
    write_sheet(ws, data, summary_format=summary_format)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--format", default="proportion_correct",
                        choices=["proportion_correct", "three_proportions"],
                        help="Summary block format (default: proportion_correct)")
    args = parser.parse_args()

    prn_to_excel(args.prn, args.xlsx, args.sheet, summary_format=args.format)
